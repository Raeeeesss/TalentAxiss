  ...
  if (active && payload?.length) {
    return (
      <div className="bg-[#0f0f1a] border border-white/10 rounded-xl px-3 py-2 text-xs">
        ...
        <div className="font-bold text-white">{payload[0].value} placements</div>
      </div>
    );
  }
  return null;
        ...
        {quickActions.map((action, i) => (
          <Link key={action.label} href={action.href}>
            ...
            >
              ...
              <span className="text-xs text-white/60 font-medium text-center">{action.label}</span>
            </motion.div>
          </Link>
        ))}
      </motion.div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="font-semibold text-white">Placements Trend</h3>
            ...
            {[
              ...
              <div key={s.stage}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-white/50">{s.stage}</span>
                ...
                <Link href={`/dashboard/candidates/${c.id}`} key={c.id}>
                  <div className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/4 transition-colors cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-white/30 truncate">{c.currentRole || "New candidate"}</div>
                    </div>
                    <div className="text-xs text-white/20">{formatDate(c.createdAt)}</div>
                  </div>
                </Link>
              ))
            ) : (
              ...
              [
                ...
                <div key={c.name} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/4 transition-colors cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-white/30">{c.role}</div>
                  </div>
        ...
        >
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div key={i} className={`flex items-start gap-3 p-3 rounded-xl border transition-colors ${task.done ? "border-white/4 opacity-40" : "border-white/6 hover:border-white/10 hover:bg-white/2"}`}>
                ...
                <span className={`text-sm flex-1 leading-tight ${task.done ? "line-through text-white/30" : "text-white/70"}`}>
                  {task.label}
                </span>
                <span className={`text-xs px-1.5 py-0.5 rounded-full border shrink-0 ${task.priority === "high" ? "bg-red-500/10 text-red-400 border-red-500/20" : task.priority === "medium" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-white/5 text-white/30 border-white/8"}`}>
                  {task.priority}
                </span>
              </div>
            ))}
