      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-8 mb-12">
          ...
          <div className="col-span-2">
            ...
            <p className="text-sm text-foreground/40 leading-relaxed mb-4 max-w-xs">
              The AI-powered recruitment CRM built for Kerala consultancy agencies.
              Transform your business with the future of hiring.
            </p>
            <div className="flex flex-col gap-2 text-xs text-foreground/30">
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm text-foreground/40">
              {["Features", "Pricing", "AI Matching", "Gulf Mode", "Changelog"].map((item) => (
                <li key={item}><Link href="#" className="hover:text-foreground transition-colors">{item}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex items-center gap-3">
            {socialIcons.map((Icon, i) => (
              <Link
                key={i}
                href="#"
                className="w-8 h-8 rounded-lg bg-white/5 border border-white/8 flex items-center justify-center text-foreground/40 hover:text-foreground hover:border-white/20 transition-colors"
              >
