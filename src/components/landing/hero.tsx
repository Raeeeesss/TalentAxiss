...
import {
  ArrowRight, Sparkles, Users, Briefcase, TrendingUp,
  CheckCircle2, Star, Zap, Brain, Play,
} from "lucide-react";
...
function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    const COUNT = typeof window !== "undefined" && window.innerWidth < 768 ? 30 : 65;
    let pts: P[] = [];
    ...
    const draw = () => {
      ...
      pts.forEach((p) => {
        ...
        p.vx = Math.max(-1.4, Math.min(1.4, p.vx * .99));
        p.vy = Math.max(-1.4, Math.min(1.4, p.vy * .99));
        p.x += p.vx; p.y += p.vy;
      ...
      <div className="device-screen bg-gray-50">
        ...
        <div className="flex items-center gap-1.5 px-3 py-2 bg-white border-b border-gray-100">
          ...
          <div className="flex-1 mx-3 h-5 bg-gray-100 rounded-md flex items-center px-2.5">
            <span className="text-[9px] text-gray-400 font-mono">app.talentaxiss.in/dashboard</span>
          </div>
        </div>
        ...
        <div className="flex h-64 sm:h-80">
          ...
          <div className="hidden sm:flex w-32 bg-white border-r border-gray-100 flex-col py-3 px-2 gap-0.5 shrink-0">
            <div className="flex items-center gap-1.5 px-2 mb-3">
              ...
              <span className="text-[8px] font-bold gradient-text">TalentAxiss</span>
            </div>
            {[
          ...
          <div className="flex-1 p-3 overflow-hidden">
            <div className="text-[9px] font-bold text-gray-700 mb-2">Good morning, Arun 👋</div>
            ...
            <div className="grid grid-cols-4 gap-1.5 mb-2.5">
              {[
                ...
                <motion.div
                  ...
                  transition={{ delay: 0.9 + Math.random() * .3 }}
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="col-span-2 bg-white rounded-xl border border-gray-100 p-2.5 h-28 shadow-sm">
                <div className="text-[8px] text-gray-400 mb-2 font-medium">Placements — 6 months</div>
                <div className="flex items-end gap-0.5 h-16">
                  {[22, 38, 30, 55, 45, 68, 52, 80, 62, 88, 72, 94].map((h, i) => (
                    <motion.div
                      key={i}
                      initial={{ height: 0 }}
                      animate={{ height: `${h}%` }}
                      transition={{ delay: 1 + i * .035, duration: .5, ease: "easeOut" }}
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-2.5 h-28 shadow-sm">
                <div className="text-[8px] text-gray-400 mb-2 font-medium">Pipeline</div>
                {[
                  ...
                  <div key={r.s} className="mb-2">
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: r.w }}
                        transition={{ delay: 1.2, duration: .7, ease: "easeOut" }}
...
export function HeroSection() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      rawX.set((e.clientX / window.innerWidth  - .5));
      rawY.set((e.clientY / window.innerHeight - .5));
    };
    window.addEventListener("mousemove", handler, { passive: true });
    return () => window.removeEventListener("mousemove", handler);
  }, [rawX, rawY]);

  const w1x = useTransform(springX, [-.5, .5], [-28, 28]);
  const w1y = useTransform(springY, [-.5, .5], [-18, 18]);
  const w2x = useTransform(springX, [-.5, .5], [22, -22]);
  const w2y = useTransform(springY, [-.5, .5], [-14, 14]);
  const w3x = useTransform(springX, [-.5, .5], [-18, 18]);
  const w3y = useTransform(springY, [-.5, .5], [16, -16]);
    ...
    >
      ...
      >
        ...
        >
          ...
          <span className="text-[10px] bg-indigo-600 text-white px-2 py-0.5 rounded-full font-bold animate-pulse-ring">NEW</span>
        </motion.div>
        ...
        <motion.h1
          ...
          className="text-5xl sm:text-6xl lg:text-[5.25rem] font-extrabold leading-[1.06] tracking-tight mb-6"
        >
          <span className="text-gray-950">Manage Candidates,</span>
          <br />
          <span className="gradient-text">AI Matching & Placements</span>
          <br />
          <span className="text-gray-800 text-4xl sm:text-5xl lg:text-6xl font-bold">in One CRM</span>
        </motion.h1>
        ...
        >
          ...
          TalentAxiss gives Kerala consultancy agencies a powerful AI-driven CRM to
          find, match, and place candidates <strong className="text-gray-700 font-semibold">10× faster.</strong>
        </motion.p>
        ...
        <motion.div
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: .6, delay: .28 }}
        ...
        >
          {["No credit card required", "7-day free trial", "Cancel anytime", "Kerala-built"].map((t) => (
        ...
        >
          {[
            { icon: Users,      value: 2847, suffix: "+", label: "Candidates placed",  grad: "from-indigo-500 to-purple-600" },
            { icon: Briefcase,  value: 156,  suffix: "+", label: "Active agencies",    grad: "from-emerald-500 to-cyan-500"  },
            { icon: TrendingUp, value: 94,   suffix: "%", label: "Placement rate",     grad: "from-amber-500 to-orange-500"  },
            { icon: Star,       value: 4.9,  suffix: "/5", label: "Agency rating",    grad: "from-rose-500 to-pink-500"     },
            ...
            >
              <div className={`w-8 h-8 rounded-xl bg-linear-to-br ${grad} flex items-center justify-center shadow-sm`}>
      ...
      >
        ...
        <motion.div style={{ x: w1x, y: w1y }} className="absolute -top-8 -left-4 sm:left-4 z-20 hidden sm:block">
          <FloatWidget delay={1.0}>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="text-xs font-bold text-gray-900">AI Match · 96%</div>
                <div className="text-[10px] text-gray-400">Rajan K. — Best Fit</div>
              </div>
        ...
        <motion.div style={{ x: w2x, y: w2y }} className="absolute -top-6 -right-4 sm:right-2 z-20 hidden sm:block">
          <FloatWidget delay={1.15}>
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                {["AM","PR","SK"].map((i) => (
                  <div key={i} className="w-6 h-6 rounded-full bg-linear-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-[8px] font-bold ring-2 ring-white">
                    {i[0]}
                  </div>
                ))}
              </div>
              <div>
                <div className="text-xs font-bold text-gray-900">3 Placed Today</div>
                <div className="text-[10px] text-emerald-600 font-medium">+₹42,000 revenue</div>
              </div>
            </div>
          </FloatWidget>
        </motion.div>
        ...
        <motion.div style={{ x: w3x, y: w3y }} className="absolute -bottom-6 left-4 sm:left-12 z-20 hidden sm:block">
          <FloatWidget delay={1.3}>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="text-xs font-bold text-gray-900">Interview @ 2:00 PM</div>
                <div className="text-[10px] text-gray-400">Priya M. · Gulf Manager Role</div>
              </div>
            </div>
          </FloatWidget>
        </motion.div>
