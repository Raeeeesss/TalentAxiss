  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-foreground/30 text-sm mt-0.5">Real-time platform overview · {new Date().toLocaleString("en-IN")}</p>
      </motion.div>
        ...
        {kpis.map((kpi, i) => (
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="text-sm text-foreground/40 mt-0.5">{kpi.label}</div>
              <div className="text-xs text-foreground/25 mt-1">{kpi.change}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-foreground/30 mt-0.5">Monthly Recurring Revenue</p>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
            <h3 className="font-semibold text-foreground">Recent Agencies</h3>
            ...
            {recentAgencies.map((agency) => (
              <div key={agency.id} className="flex items-center justify-between px-5 py-3 hover:bg-white/2 transition-colors">
                <div>
                  ...
                  <div className="text-xs text-foreground/30">{agency.owner} · {agency.district}</div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-foreground/20 w-12 text-right">{agency.joined}</span>
                </div>
              </div>
            ))}
          </div>
              ...
              {[
                ...
                <Link key={action.label} href={action.href}>
                  <div className={`flex items-center justify-between p-3 rounded-xl border border-white/6 hover:border-white/12 hover:bg-white/2 transition-all cursor-pointer`}>
                    ...
                    <ArrowRight className="h-3.5 w-3.5 text-foreground/20" />
                  </div>
                </Link>
              ))}
            </div>
